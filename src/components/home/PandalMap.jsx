...
// Three columns -- title/tagline/CTA, the map itself, and the category
// legend -- laid out side by side instead of overlaid on the map with
// gradient scrims. Keeps the section's original dark bg-ink + gold/white
...
export default function PandalMap({ content }) {
  ...
  // the viewport, so picking one from the legend visibly "takes you there"
  // on the map instead of just highlighting something off-screen.
  ...
  return (
    <section className="relative overflow-hidden bg-ink py-16 text-white sm:py-20">
      <div className="mx-auto max-w-[1400px] px-6 sm:px-10 lg:px-16 xl:px-20">
        {/* Header: title + tagline + CTA, full width above the map. */}
        <div className="mx-auto max-w-xl text-center">
          <h2 className="font-display text-2xl font-bold uppercase leading-tight text-gold sm:text-3xl">
            {title[0]}
            <span className="block text-white">{title[1]}</span>
          </h2>
          <p className="mt-3 text-sm leading-relaxed text-white/65">{tagline}</p>
          <GoldButton href={cta.href} className="mt-6">
            {cta.label}
            <ArrowRightIcon className="size-4" />
          </GoldButton>
        </div>

        <div className="mt-12 grid gap-8 lg:grid-cols-[minmax(0,1fr)_300px] lg:items-start">
          {/* Map: a card of its own -- nothing overlaid on it besides a
          ...
          {/* Right: legend, its own column -- not overlaid on the map. */}
